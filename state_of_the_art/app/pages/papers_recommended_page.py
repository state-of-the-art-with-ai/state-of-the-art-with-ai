...
with st.expander("Recommendations options"):
    lookback_days = st.number_input("Lookback days", value=1, min_value=1, max_value=365)

    if st.button("Generate new recommendations"):
        with st.spinner("Generating new recommendations"):
            InterestsRecommender().generate(skip_register_new_papers=True, number_of_days_to_look_back=lookback_days, ignore_last_recommendation=True, repeat_check_disable=True)

papers = []
for interest, interest_data in structured['interest_papers'].items():
        for paper in PapersLoader().load_papers_from_urls(interest_data['papers'].keys())[0:PAPER_PER_TOPIC_TO_RENDER]:
            papers.append(paper)
...
for topic, metadata in structured['interest_papers'].items():
    for paper in metadata["papers"]:
        papers_metadata[paper] = {"labels": [topic]} if paper not in papers_metadata else {"labels": papers_metadata[paper]["labels"] + [topic]}

render_papers(papers, papers_metadata=papers_metadata, generated_date=generated_date, metadata=recommendation_metadata)
